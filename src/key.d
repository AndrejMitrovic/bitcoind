...
public:
    ...
    bool SetPrivKey(const ubyte[] vchPrivKey)
    {
    	...
    	if (!d2i_ECPrivateKey(&pkey,
            cast(const(ubyte)**)&pbegin, cast(ubyte**)vchPrivKey.length))
        {
    		return false;
    	}
    ...
    bool isValid()
    {
    	if (!EC_KEY_check_key(pkey))
    		throw new Exception("CKey::isValid() : EC_KEY_check_key failed");
    ...
    bool Sign(Uint256 hash, ubyte[] vchSig)
    {
        ...
        ubyte[10000] pchSig;
        uint nSize = 0;
        if (!ECDSA_sign(0, cast(ubyte*)&hash, hash.sizeof, cast(ubyte*)pchSig,
            &nSize, pkey))
            return false;
    ...
    bool Verify(Uint256 hash, ubyte[] vchSig)
    {
        ...
        if (ECDSA_verify(0, (cast(ubyte*)&hash), hash.sizeof, &vchSig[0],
            cast(int)vchSig.length, pkey) != 1)
            return false;
